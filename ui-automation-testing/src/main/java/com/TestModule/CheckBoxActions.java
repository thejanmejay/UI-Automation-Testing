...
public class CheckBoxActions {
    ...
    public void selectChildCheckboxes() {
        ...
        if (!desktop.isSelected()) desktop.click();

        WebElement documents = driver.findElement(By.id("tree-node-documents"));
        if (!documents.isSelected()) documents.click();

        WebElement downloads = driver.findElement(By.id("tree-node-downloads"));
        if (!downloads.isSelected()) downloads.click();

        System.out.println("Child checkboxes selected.");
    }

    public void expandDocumentAndDownload() {
        WebElement docToggle = driver.findElement(By.xpath("//label[@for='tree-node-documents']/preceding-sibling::button"));
        docToggle.click();
        WebElement downToggle = driver.findElement(By.xpath("//label[@for='tree-node-downloads']/preceding-sibling::button"));
        downToggle.click();
